...
async function obtenerToken() {
  ...
  if (!publicKey || !username || !password || !userId) throw new Error('Credenciales Tango no configuradas');
  if (cachedToken && tokenExpiry && Date.now() < tokenExpiry) return cachedToken;
  logger.info('Obteniendo token de Tango Factura...');
  const response = await axios.post('https://www.tangofactura.com/api/Token',
...
async function enviarATango(venta) {
  ...
  try {
    ...
    const response = await axios.post(`${TANGO_BASE_URL}/CrearFactura`, payload,
      { headers: { 'Content-Type': 'application/json' }, timeout: 15000 }
    );
    const tangoId = response.data?.MovimientoID || response.data?.Id || 'OK';
    logger.info(`Remito ${venta.numero} enviado a Tango — ID: ${tangoId}`);
    await prisma.venta.update({ where: { id: venta.id }, data: { tangoRemitoId: String(tangoId), tangoEnviado: true } });
    await prisma.eventoVenta.create({ data: { ventaId: venta.id, tipo: 'TANGO_ENVIADO', detalle: `ID ${tangoId}` } });
    return response.data;
  } catch (err) {
    if (err.response?.status === 401) { cachedToken = null; tokenExpiry = null; return enviarATango(venta); }
    const msg = err.response?.data?.Message || err.response?.data?.message || err.message;
    logger.error(`Error Tango ${venta.numero}: ${msg}`);
    await prisma.eventoVenta.create({ data: { ventaId: venta.id, tipo: 'TANGO_ERROR', detalle: `Error: ${msg}` } });
    throw err;
  }
}
...
function buildTangoPayload(venta, token) {
  return {
    ...
    Observaciones: [venta.notas, venta.transporteNombre ? `Transporte: ${venta.transporteNombre}` : null, `Canal: ${venta.canal}`, `Ref: ${venta.numero}`].filter(Boolean).join(' | ')
  };
}

async function reintentarPendientes() {
  const pendientes = await prisma.venta.findMany({ where: { tangoEnviado: false, estado: { not: 'CANCELADO' } }, include: { items: true }, take: 20 });
  for (const v of pendientes) await enviarATango(v).catch(() => {});
}
